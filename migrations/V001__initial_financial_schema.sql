-- ============================================
-- MilkyHoop 4.0 - Financial Schema Migration
-- Version: V001__initial_financial_schema.sql
-- Date: 2025-10-29
...
CREATE POLICY tenant_isolation_outbox ON outbox
    ...
    USING (
        EXISTS (
            ...
            AND transaksi_harian.tenant_id::text = current_setting('app.current_tenant_id', TRUE)
        )
    );
...
-- ============================================
-- Example: Create default tax_info for existing tenants
-- INSERT INTO tax_info (tenant_id, periode, status_wp)
-- SELECT id, TO_CHAR(NOW(), 'YYYY-MM'), 'orang_pribadi'
-- FROM "Tenant"
...
-- AND tablename IN ('transaksi_harian', 'outbox', 'tax_info');
...
-- WHERE schemaname = 'public' AND tablename = 'transaksi_harian';
