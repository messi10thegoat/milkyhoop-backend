...
class FixedEnhancedTenantParserService(pb_grpc.IntentParserServiceServicer):
    ...
    async def CalculateConfidence(self, request: pb.ConfidenceRequest, context) -> pb.ConfidenceResponse:
        ...
        try:
            ...
            response.tier_number = int(tier_num)
    ...
    async def GetPoliteDeflection(self, request: pb.DeflectionRequest, context) -> pb.DeflectionResponse:
        ...
        try:
            ...
            # FIXED: Build response with safe string conversion
            response = pb.DeflectionResponse()
            default_message = "Maaf, pertanyaan ini belum bisa saya jawab. Silakan hubungi customer service untuk bantuan lebih lanjut."
            response.message = str(deflection_messages.get(tenant_id, deflection_messages.get("default", default_message)))
