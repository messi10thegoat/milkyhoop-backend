...
| WIP_GENERIC | 1-10650 | Work-in-Progress (unified) | ASSET — D3.1 catalog promote (V158) + **MAPPED (5/5) via V159 Fase D3.2**. Single WIP bucket: semua biaya produksi (raw+labor+overhead) unified di sini, tidak dipisah granular. Future split via `WIP_RAW/WIP_LABOR/WIP_OVERHEAD` reservasi. Akan flips literal di `production.py` saat D3.3. V159 backfilled 1-10650 ke 4 tenant missing (anthonius-iwan, milkytest, ponte-publishing, potus-id) — fixes live HTTP 500 on Work Order release. |
| COGS_VARIANCE_PRODUCTION | 5-90200 | HPP Varian Produksi (lumped) | OTHER_EXPENSE / DEBIT — D3.1 catalog promote (V158) + **MAPPED (5/5) via V159 Fase D3.2**. Varian total material+labor+overhead lumped (tidak granular per cost element). Existing granular `COGS_VARIANCE_MATERIAL/LABOR/OVERHEAD` tetap reserved untuk future per-element variance. V159 backfilled 5-90200 "Selisih Produksi" ke 4 tenant missing. |
| WIP_SUBCONTRACT | 1-10650 | Biaya Subkontrak/Maklon | ASSET — D3.1 catalog promote (V158) + **MAPPED (5/5) via V159 Fase D3.2**. Subcontract/maklon biaya routes ke akun WIP yang sama (`1-10650`) — per recon Step 1 keputusan owner: tidak pisah code. Akan flips ternary `bills_service.py:3340-3435` literal saat D3.3. |
| INVENTORY_ADJUSTMENT_EXPENSE | 5-50100 | Biaya Penyesuaian Persediaan | EXPENSE / DEBIT — D3.1 catalog promote (V158) + **MAPPED (5/5) via V159 Fase D3.2**. Generic stock adjustment loss; akan flips literal di `stock_adjustments.py` saat D3.3. V159 backfilled 5-50100 ke 3 tenant missing (anthonius-iwan, ponte-publishing, potus-id; milkytest already had it). |
| SALARY_EXPENSE | 5-20100 | Beban Gaji | EXPENSE / DEBIT — D4.1 catalog promote (V161) + **MAPPED (7/7) via V162 Fase D4.2**. Existing code di seed_default_coa. Flips literal `payroll.py:65` `SALARY_EXPENSE_ACCOUNT = "6-10100"` (non-existent code → bug live: golden-apparel POST gagal) + parallel const `payroll_calc.py:52`. |
| SALARY_PAYABLE | 2-10400 | Utang Gaji | LIABILITY / CREDIT — D4.1 catalog promote (V161) + **MAPPED (7/7) via V162 Fase D4.2**. Existing code di seed_default_coa. Flips literal `payroll.py:66` `SALARY_PAYABLE_ACCOUNT = "2-10500"` (semantically wrong: 2-10500 = Uang Muka Pelanggan, customer deposit, NOT salary payable). |
| PPH21_PAYABLE | 2-10310 | Utang PPh 21 | LIABILITY / CREDIT — D4.1 catalog promote (V161) + **MAPPED (7/7) via V162 Fase D4.2**. **PAYROLL-EXCLUSIVE BOUNDARY** (LOCKED §"PPH 21 PAYROLL BOUNDARY"): jangan resolve lewat WHT_PPH_PAYABLE (2-10320 = AP transaksi). V162 backfilled 2-10310 ke 3 tenant missing (golden-apparel, golden-verify, potus-id). |
| BPJS_EE_PAYABLE | 2-10410 | Utang BPJS Karyawan | LIABILITY / CREDIT — D4.1 catalog promote (V161) + **MAPPED (7/7) via V162 Fase D4.2**. V162 backfilled 2-10410 ke 3 tenant missing. |
| BPJS_ER_PAYABLE | 2-10420 | Utang BPJS Perusahaan | LIABILITY / CREDIT — D4.1 catalog promote (V161) + **MAPPED (7/7) via V162 Fase D4.2**. V162 backfilled 2-10420 ke 3 tenant missing. |
| BPJS_ER_EXPENSE | 5-20150 | Beban BPJS Perusahaan | EXPENSE / DEBIT — D4.1 catalog promote (V161) + **MAPPED (7/7) via V162 Fase D4.2**. V162 backfilled 5-20150 ke 3 tenant missing. |
| PPH21_ER_EXPENSE | 5-80100 | Beban PPh 21 Perusahaan | EXPENSE / DEBIT — D4.1 catalog promote (V161) + **MAPPED (7/7) via V162 Fase D4.2**. Nett method only (kalau employer tanggung PPh 21). V162 backfilled 5-80100 ke 3 tenant missing. |
