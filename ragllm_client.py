...
class RAGLLMClient:
    ...
    async def generate_response(
        ...
        try:
            ...
            request = pb.GenerateAnswerRequest(
                ...
                model=model,
                temperature=0.7 if intelligence_level == "synthesis" else 0.5,
                max_tokens=300 if intelligence_level == "synthesis" else 500
            )
    ...
    async def synthesize_with_context(
        ...
        try:
            ...
            response = await self.stub.SynthesizeResponse(request)
    ...
    def _format_faq_context(self, faq_results: List[Any]) -> str:
        ...
        context_parts = []
        for i, faq in enumerate(faq_results[:4], 1):  # Max 4 FAQs for context
            question = getattr(faq, 'question', '')
            answer = getattr(faq, 'answer', '')
            content = getattr(faq, 'content', '')
            
            # Use content if available, otherwise combine question + answer
            faq_text = content if content else f"Q: {question}\nA: {answer}"
