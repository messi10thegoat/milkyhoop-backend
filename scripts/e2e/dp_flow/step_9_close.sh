...
echo "  journal_entries: $JE_BEFORE -> $JE_AFTER"
echo "  bank_transactions: $BT_BEFORE -> $BT_AFTER"
FINALSTATUS=$(PSQL "SELECT status FROM sales_orders WHERE id='$SOID';")
echo "  final status: $FINALSTATUS | shipped_qty: $(PSQL "SELECT shipped_qty::bigint FROM sales_orders WHERE id='$SOID';") (STAYS 0 — finding #4, close does not reconcile) | invoiced_qty: $(PSQL "SELECT invoiced_qty::bigint FROM sales_orders WHERE id='$SOID';")"
aeq "SO close created ZERO new journal" "$JE_AFTER" "$JE_BEFORE"
aeq "SO close moved no bank" "$BT_AFTER" "$BT_BEFORE"
aeq "SO status completed" "$FINALSTATUS" "completed"
