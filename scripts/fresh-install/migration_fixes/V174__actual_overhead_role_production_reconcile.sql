...
BEGIN
    ...
    IF v_ga_code IS NULL THEN
        RAISE NOTICE 'V174 verify-gate SKIPPED (fresh install): golden-apparel MFG_ACTUAL_OVERHEAD mapping missing';
    END IF;
    IF v_ga_code <> '5-30300' THEN
        RAISE NOTICE 'V174 verify-gate SKIPPED (fresh install): golden-apparel MFG_ACTUAL_OVERHEAD -> % (expected 5-30300)', v_ga_code;
    END IF;
    ...
    IF v_src_ok <> 1 THEN
        RAISE NOTICE 'V174 verify-gate SKIPPED (fresh install): PRODUCTION_RECONCILE not registered in journal_source_types (found %)', v_src_ok;
    END IF;
    ...
    IF v_missing > 0 THEN
        RAISE NOTICE 'V174 verify-gate SKIPPED (fresh install): % tenant(s) with 5-30300 missing MFG_ACTUAL_OVERHEAD', v_missing;
    END IF;
