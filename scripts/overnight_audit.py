...
def test_endpoint(
    method: str, path: str, token: str, data: dict = None, expected_status: int = 200
) -> dict:
...
def get_auth_token() -> str:
    ...
    resp = requests.post(
        ...
        json={"email": "test@milkyhoop.com", "password": "testpassword"},
    )
...
def test_reports(token: str) -> Dict[str, Any]:
    ...
    log(
        f"GET /reports/trial-balance: {r['status_code']}",
    ...
    log(
        f"GET /reports/laba-rugi: {r['status_code']}",
        "PASS" if r["success"] else "FAIL",
    )
    ...
    log(
        f"GET /reports/arus-kas: {r['status_code']}", "PASS" if r["success"] else "FAIL"
    )

    r = test_endpoint("GET", "/reports/ar-aging", token)
    results["ar_aging"] = r["success"]
    log(
        f"GET /reports/ar-aging: {r['status_code']}", "PASS" if r["success"] else "FAIL"
    )

    r = test_endpoint("GET", "/reports/ap-aging", token)
    results["ap_aging"] = r["success"]
    log(
        f"GET /reports/ap-aging: {r['status_code']}", "PASS" if r["success"] else "FAIL"
    )
...
def test_receive_payments(token: str) -> Dict[str, Any]:
    ...
    results["list"] = r["success"]
    log(
        f"GET /receive-payments: {r['status_code']}", "PASS" if r["success"] else "FAIL"
    )

    r = test_endpoint("GET", "/receive-payments/summary", token)
    results["summary"] = r["success"]
    log(
        f"GET /receive-payments/summary: {r['status_code']}",
        "PASS" if r["success"] else "FAIL",
    )
...
def test_expenses(token: str) -> Dict[str, Any]:
    ...
    results["summary"] = r["success"]
    log(
        f"GET /expenses/summary: {r['status_code']}", "PASS" if r["success"] else "FAIL"
    )
...
def test_bank(token: str) -> Dict[str, Any]:
    ...
    log(
        f"GET /bank-reconciliation/sessions: {r['status_code']}",
        "PASS" if r["success"] else "FAIL",
    )
