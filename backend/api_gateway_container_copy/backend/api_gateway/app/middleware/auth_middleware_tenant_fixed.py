...
Path classification bug resolved + Tenant routes support
"""
...
class AuthMiddleware(BaseHTTPMiddleware):
    """
    Authentication Middleware with FIXED path segregation + Tenant support
    """
    ...
    def __init__(self, app, auth_service_host: str = "auth_service:5004"):
        ...
        self.public_paths = [
            ...
            "/api/auth/register",    # ✅ FIX: Register endpoint must be public
            "/tenant/",              # ✅ TENANT: All tenant routes public
            "/tenant/bca/chat",      # ✅ TENANT: Specific BCA chatbot route
            "/tenant/bca/",          # ✅ TENANT: BCA tenant sub-routes
        ]
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # TENANT PATHS - check for tenant routes first (CRITICAL)
        if request.url.path.startswith("/tenant/"):
            logger.info("🌐 Tenant route detected - public access", path=request.url.path)
            return await call_next(request)
