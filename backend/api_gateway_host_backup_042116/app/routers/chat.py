...
async def chat_endpoint(req: ChatRequest):
    ...
    try:
        ...
        chatbot_response = await chatbot_client.send_message(
...
async def customer_chat_endpoint(tenant_id: str, req: CustomerChatRequest):
    """🚀 ENHANCED Customer Mode Chat - WITH CONFIDENCE ENGINE"""
    ...
    try:
        ...
        print(f"🎯 Confidence Engine: ACTIVE")
        print(f"{'='*60}")
        ...
        try:
            ...
            print(f"  🚀 Enhanced tenant_parser: ✅ CALLED")
...
# SURGICAL FIX: Explicit GET endpoint for authentication testing
@router.get("/auth-test")
async def chat_auth_test_explicit():
    """
    Explicit GET endpoint for Bearer token authentication testing
    Protected endpoint - requires Authorization: Bearer <token>
    """
    return {
        "message": "Authentication successful - GET method working",
        "endpoint": "/chat/auth-test",
        "method": "GET",
        "authentication": "Bearer token validated",
        "user_context": "Available via request.state.user",
        "phase": "Phase 2 - Authentication Fixed"
    }
