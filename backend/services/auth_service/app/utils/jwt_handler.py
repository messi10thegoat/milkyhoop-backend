...
class JWTHandler:
    ...
    def create_access_token(
        ...
        Args:
            ...
            email: User email
            username: Username
            device_id: Device identifier for session enforcement
        ...
        try:
            ...
            logger.info(
                f"Access token created for user: {user_id}, tenant: {tenant_id}, device: {device_type}"
            )
            return token
    ...
    def create_refresh_token(
        ...
        try:
            ...
            token = jwt.encode(payload, JWT_SECRET, algorithm=JWT_ALGORITHM)
            logger.info(
                f"Refresh token created for session: {session_id}, device: {device_type}"
            )
            return token
    ...
    def verify_token(
        ...
        try:
            ...
            logger.info(
                f"Token verified successfully for user: {payload.get('user_id')}"
            )
            return payload
...
def create_access_token(
    ...
    return JWTHandler.create_access_token(
        user_id, tenant_id, role, email, username, device_id, device_type
    )
...
def create_refresh_token(
    ...
    return JWTHandler.create_refresh_token(
        user_id, session_id, tenant_id, device_id, device_type
    )
