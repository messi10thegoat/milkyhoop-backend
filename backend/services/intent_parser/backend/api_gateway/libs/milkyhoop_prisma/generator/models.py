...
class ModelExtension(BaseModel):
    ...
    def instance_name_validator(cls, name: Optional[str]) -> Optional[str]:
        ...
        if not name.isidentifier():
            raise ValueError(
                f'Custom Model instance_name "{name}" is not a valid Python identifier'
            )
...
class Model(BaseModel):
    ...
    def validate_dsl_extension(cls, values: Dict[Any, Any]) -> Dict[Any, Any]:
        ...
        if parsed["type"] == "ok":
            values["extension"] = model_parse(
                ModelExtension, parsed["value"]["arguments"]
            )
...
class Field(BaseModel):
    ...
    def where_aggregates_input_type(self) -> str:
        ...
        if typ in FILTER_TYPES:
            return (
                f"Union[{self._actual_python_type}, 'types.{typ}WithAggregatesFilter']"
            )
        return self.python_type
