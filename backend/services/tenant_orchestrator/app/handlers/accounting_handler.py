"""
Accounting Handler for Tenant Orchestrator
Handles accounting journal queries
...
class AccountingHandler:
    """
    Static class for accounting-related operations in tenant mode
    ...
    async def handle_accounting_query(
        ...
        """
        Handle accounting journal query - returns string response
        ...
        # Parse entities
        try:
            accounting_entities = json.loads(intent_response.entities_json)
        ...
        if query_type != "journal_entries":
            return f"Fitur lihat {query_type} belum tersedia. Coba 'cek jurnal bulan ini'?"
        ...
        try:
            ...
            return f"Maaf, gagal ambil jurnal. Error: {e.details()[:100]}"
        except Exception as e:
            logger.error(f"[{trace_id}] Unexpected error: {e}")
            return f"Ada kendala ambil jurnal. Error: {str(e)[:100]}"