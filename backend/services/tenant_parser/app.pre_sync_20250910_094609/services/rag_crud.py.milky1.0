...
SEMANTIC_SIMILARITY_THRESHOLD = 0.55  # 75% semantic similarity
...
def calculate_dynamic_threshold(query_length: int, has_exact_match: bool = False) -> float:
    """Calculate optimal threshold based on query characteristics"""
    base_threshold = 0.55
...
async def fuzzy_search_rag_documents(
    ...
    # Generate query embedding once (optimization)
    ...
    for doc in docs:
        ...
        if query_embedding is not None:
            try:             
                ...
                if result and result[0]['embeddings']:
                    doc_embedding = np.array(result[0]['embeddings'] if isinstance(result[0]['embeddings'], list) else json.loads(result[0]['embeddings']))
                else:
