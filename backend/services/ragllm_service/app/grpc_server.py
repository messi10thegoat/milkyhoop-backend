...
class RagLlmServiceServicer(pb_grpc.RagLlmServiceServicer):
    ...
    async def GenerateAnswer(self, request, context):
        ...
        try:
            ...
            faq_context = getattr(request, 'faq_context', '')
            intelligence_level = getattr(request, 'intelligence_level', 'synthesis')
            model = getattr(request, 'model', 'gpt-3.5-turbo')
            
            logger.info(f"🎯 FAQ Context provided: {len(faq_context)} chars")
            logger.info(f"🎯 Intelligence Level: {intelligence_level}")
            ...
            # NEW: Use provided FAQ context instead of internal search
            if faq_context and len(faq_context) > 50:
...
A:"""
                ...
                # Fallback to existing logic for non-customer queries
