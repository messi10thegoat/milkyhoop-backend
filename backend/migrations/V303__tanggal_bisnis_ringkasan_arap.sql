-- V303 (#10b-3a) — "hari ini" di fungsi inti AR/AP = tanggal BISNIS tenant, bukan UTC.
--
-- Postgres berjalan UTC: pukul 00.00–07.00 WIB CURRENT_DATE = KEMARIN. Sisi tulis
-- sudah bertanggal bisnis (#10a/#10b, V302 tanggal_bisnis); sisi baca aplikasi
-- (#10b-3a) kini membandingkan due_date dengan tanggal bisnis. Fungsi ini tertinggal:
--   * compute_ar_summary / compute_ap_summary memisah overdue vs current pakai CURRENT_DATE.
--   * compute_ar_outstanding / compute_ap_outstanding memberi baris sintetis (nota kredit /
--     vendor credit belum diterapkan, bersaldo NEGATIF) due_date = CURRENT_DATE. Dibaca
--     dengan tanggal bisnis pada 00–07 WIB, baris itu jadi "lewat jatuh tempo" → jumlah
--     overdue berkurang palsu.
-- Isi = definisi LIVE (pg_get_functiondef 25 Sep 2026) dengan SATU substitusi:
-- CURRENT_DATE -> tanggal_bisnis(p_tenant_id). Tanpa perubahan lain (diff diperiksa).
-- Trigger update_sales_invoice_status / update_bill_on_payment (CURRENT_DATE) SENGAJA
-- tidak disentuh: menempel di tabel lama sales_invoice_payments / bill_payments,
-- 0 baris, 0 penulis di kode aplikasi.
...
CREATE OR REPLACE FUNCTION public.compute_ar_summary(p_tenant_id text)
 RETURNS TABLE(total_outstanding numeric, customer_count bigint, overdue_amount numeric, current_amount numeric)
 LANGUAGE sql
 STABLE
AS $function$
    SELECT COALESCE(SUM(outstanding), 0)::NUMERIC(18,2), COUNT(DISTINCT customer_id),
           COALESCE(SUM(CASE WHEN due_date < tanggal_bisnis(p_tenant_id) THEN outstanding ELSE 0 END), 0)::NUMERIC(18,2),
           COALESCE(SUM(CASE WHEN due_date >= tanggal_bisnis(p_tenant_id) THEN outstanding ELSE 0 END), 0)::NUMERIC(18,2)
...
CREATE OR REPLACE FUNCTION public.compute_ap_summary(p_tenant_id text)
 RETURNS TABLE(total_outstanding numeric, vendor_count bigint, overdue_amount numeric, current_amount numeric)
 LANGUAGE sql
 STABLE
AS $function$
