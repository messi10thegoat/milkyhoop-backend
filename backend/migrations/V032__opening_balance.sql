-- ============================================================================
-- V032: Opening Balance Mechanism
-- ============================================================================
...
-- Insert Opening Balance Equity account for all tenants that have CoA
...
SELECT
    ...
    'EQUITY',
    'CREDIT',
    (SELECT id FROM chart_of_accounts c2 WHERE c2.tenant_id = t.tenant_id::uuid AND c2.code = '3-00000' LIMIT 1),
    true,
    true
FROM (SELECT DISTINCT tenant_id FROM chart_of_accounts) t
WHERE NOT EXISTS (
    SELECT 1 FROM chart_of_accounts
    WHERE code = '3-50000' AND tenant_id = t.tenant_id::uuid
);
...
CREATE TABLE IF NOT EXISTS opening_balance_records (
    ...
    -- Linked journals
    gl_journal_id UUID REFERENCES journal_entries(id),     -- Main GL opening balance journal
    ar_journal_id UUID,                                     -- AR subledger journal (if separate)
    ap_journal_id UUID,                                     -- AP subledger journal (if separate)
    inventory_journal_id UUID,                              -- Inventory journal (if separate)
...
BEGIN
    ...
    WHERE tenant_id = p_tenant_id::uuid AND code = '3-50000';
