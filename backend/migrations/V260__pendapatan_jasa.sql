-- V260: Pendapatan Jasa (4-10150) + REVENUE_SALES_SERVICE map. Idempoten. Law 19: nol jurnal disentuh.
...
-- Existing tenants: sisip 4-10150 "Pendapatan Jasa" idempoten (semua tenant dummy).
INSERT INTO chart_of_accounts (tenant_id, account_code, name, account_type, normal_balance, parent_code, level, is_header)
SELECT DISTINCT c.tenant_id, '4-10150', 'Pendapatan Jasa', 'REVENUE', 'CREDIT', '4-10000', 2, false
FROM chart_of_accounts c
WHERE c.account_code = '4-10100'
  AND NOT EXISTS (SELECT 1 FROM chart_of_accounts c2 WHERE c2.tenant_id = c.tenant_id AND c2.account_code = '4-10150');

-- Map REVENUE_SALES_SERVICE -> 4-10150 idempoten.
INSERT INTO account_roles (tenant_id, role_key, account_id)
SELECT coa.tenant_id, 'REVENUE_SALES_SERVICE', coa.id
FROM chart_of_accounts coa
WHERE coa.account_code = '4-10150'
  AND NOT EXISTS (SELECT 1 FROM account_roles ar WHERE ar.tenant_id = coa.tenant_id AND ar.role_key = 'REVENUE_SALES_SERVICE');
