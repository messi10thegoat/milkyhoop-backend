...
class OrchestratorClient:
    ...
    def __init__(
        self, host: str = "milkyhoop-dev-cust_orchestrator-1", port: int = 5013
    ):
    ...
    async def process_customer_query(
        self, message: str, tenant_id: str, session_id: Optional[str] = None
    ) -> str:
        """
        ...
        try:
            ...
            from milkyhoop_protos.cust_orchestrator_pb2 import (
                ProcessCustomerQueryRequest,
            )
            from milkyhoop_protos.cust_orchestrator_pb2_grpc import (
                CustOrchestratorServiceStub,
            )
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                try:
                    ...
                    logger.info(
                        f"[Real gRPC] Calling ProcessCustomerQuery on attempt {attempt + 1}"
                    )

                    response = await asyncio.wait_for(
                        stub.ProcessCustomerQuery(request), timeout=self.timeout
                    )
                    ...
                    if response.response:
                        ...
                        if (
                            hasattr(response, "superintelligent_metadata")
                            and response.superintelligent_metadata
                        ):
                            meta = response.superintelligent_metadata
                            logger.info(
                                f"[SuperIntelligent] Tier {meta.tier}, Route: {meta.route}, "
                ...
                if attempt == self.max_retries:
                    ...
                    raise Exception(
                        f"SuperIntelligent orchestrator timeout after {self.timeout}s"
                    )
                else:
                    logger.warning(
                        f"[Real gRPC] Timeout on attempt {attempt + 1}, retrying..."
                    )
                ...
                if attempt == self.max_retries:
                    ...
                    raise Exception(
                        f"SuperIntelligent orchestrator gRPC error: {e.details()}"
                    )
                else:
                    logger.warning(
                        f"[Real gRPC] gRPC error on attempt {attempt + 1}: {e.details()}"
                    )
                    await asyncio.sleep(0.5 * (attempt + 1))
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                await asyncio.wait_for(stub.HealthCheck(Empty()), timeout=3.0)
    ...
    async def get_service_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                response = await asyncio.wait_for(
                    stub.GetServiceInfo(Empty()), timeout=5.0
                )
