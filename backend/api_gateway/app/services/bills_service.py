...
class BillCalculator:
    ...
    def calculate(
        ...
        if invoice_discount_percent > 0:
            invoice_discount_total = int(after_item_discount * invoice_discount_percent / 100)
        else:
        ...
        if cash_discount_percent > 0:
            cash_discount_total = int(after_invoice_discount * cash_discount_percent / 100)
        else:
    ...
    @staticmethod
    def calculate_item_total(qty: int, price: int, discount_percent: Decimal) -> Dict[str, int]:
        """
    ...
    async def create_bill_v2(
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                if not invoice_number:
                    invoice_number = await conn.fetchval(
                        "SELECT generate_purchase_bill_number($1)",
                        tenant_id
                    )
                ...
                if vendor_id:
                    ...
                    if existing_vendor:
                        ...
                        vendor_id = await conn.fetchval("""
                            ...
                            VALUES ($1, $2, $3)
                            RETURNING id
                        """, tenant_id, vendor_name, user_id)
                        logger.info(f"Auto-created vendor: {vendor_id}, name={vendor_name}")
                else:
                    return {
                        "success": False,
                ...
                calc = BillCalculator.calculate(
                    items=items,
                    invoice_discount_percent=Decimal(str(request.get('invoice_discount_percent', 0))),
                    invoice_discount_amount=request.get('invoice_discount_amount', 0),
                    cash_discount_percent=Decimal(str(request.get('cash_discount_percent', 0))),
                ...
                for idx, item in enumerate(items, start=1):
                    ...
                    discount_pct = Decimal(str(item.get('discount_percent', 0)))
                    item_calc = BillCalculator.calculate_item_total(qty, price, discount_pct)
                ...
                if status == 'posted':
                    if self.accounting:
                        ...
                        if not ap_result.get('success'):
                            raise ValueError(f"AP creation failed: {ap_result.get('error')}")
                        ...
                        await conn.execute("""
                            ...
                            WHERE id = $4
                        """, ap_id, journal_id, user_id, bill_id)
                    else:
                        # Log warning but allow draft-like behavior
                        logger.warning(f"Accounting kernel not available. Bill {bill_id} posted without AP.")

                logger.info(f"Bill V2 created: {bill_id}, status={status}, grand_total={calc['grand_total']}")
        ...
        async with self.pool.acquire() as conn:
            ...
            async with conn.transaction():
                ...
                if items:
                    ...
                    # Delete existing items
                    await conn.execute("DELETE FROM bill_items WHERE bill_id = $1", bill_id)
                ...
                if calc:
                    for field in ['subtotal', 'item_discount_total', 'invoice_discount_total',
                                  'cash_discount_total', 'dpp', 'tax_amount', 'grand_total']:
    ...
    async def get_bill_v2(
        ...
        async with self.pool.acquire() as conn:
            ...
            return {
                ...
                "invoice_discount_percent": float(bill['invoice_discount_percent'] or 0),
                ...
                "items": [
                    {
                        ...
                        "product_name": item['product_name'] or item.get('linked_product_name'),
                        ...
                        "exp_date": item['exp_date'].strftime('%Y-%m') if item['exp_date'] else None,
