...
from .tool_registry import get_tools, is_action_tool
...
def _format_thinking_label(function_name: str, arguments_json: str = "") -> str:
    ...
    entity_name = args.get("query") or args.get("search") or args.get("name") or args.get("q") or ""
...
def _get_thinking_badge(function_name: str, success: bool = True):
    ...
    VERIFIED_TOOLS = {
        "propose_direct_action", "propose_action",
        "agentic_reconcile", "confirm_single_match",
        "categorize_statement", "start_workflow",
    }
...
MUST_SEQUENTIAL_TOOLS = {
    "propose_action", "simulate_action", "propose_direct_action",  # ACTION tools
    "start_workflow", "review_next_unmatched",                      # WORKFLOW tools
}
...
class UnifiedAgent:
    ...
    async def process_message(
        ...
        complexity = tier_to_complexity.get(model_choice.tier, TaskComplexity.SIMPLE_READ)
        logger.info(f"[MODEL] tier={model_choice.tier} reason='{model_choice.reason}' -> complexity={complexity.value}")
        ...
        for iteration in range(MAX_ITERATIONS):
            ...
            if llm_response.finish_reason == "stop" or not llm_response.tool_calls:
                ...
                await emit("THINKING_DONE", {
                    "summary": _format_thinking_label(_last_tool) if _last_tool else "Menyusun jawaban",
                    "total_ms": int((_time.monotonic() - _process_start) * 1000),
                })
            ...
            messages.append(LLMMessage(
                ...
                tool_calls=llm_response.raw_message.get("tool_calls") if llm_response.raw_message else None,
            ))
            ...
            parallel_tcs = [tc for tc in all_tool_calls if tc.function_name not in MUST_SEQUENTIAL_TOOLS]
            sequential_tcs = [tc for tc in all_tool_calls if tc.function_name in MUST_SEQUENTIAL_TOOLS]
            ...
            if parallel_tcs:
                ...
                for _ptc in parallel_tcs:
                    _ptc_args_json = _json_helpers.dumps(_ptc.arguments, default=str) if _ptc.arguments else ""
                    await emit("THINKING_STEP", {
                        "step_id": _ptc.id,
                        "text": _format_thinking_label(_ptc.function_name, _ptc_args_json),
                        "status": "running",
                    })

                async def _exec_one(tc_item):
                    t0 = time.time()
                    res = await tool_executor.execute(tc_item.function_name, tc_item.arguments)
                ...
                logger.info(f"[PERF] parallel_tools={len(parallel_tcs)} total_ms={par_ms}")
                ...
                for item in par_results:
                    ...
                    _ptc_done_args = _json_helpers.dumps(tc.arguments, default=str) if tc.arguments else ""
                    await emit("THINKING_STEP", {
                        "step_id": tc.id,
                        "text": _format_thinking_label(tc.function_name, _ptc_done_args),
                        "status": "done",
                        "duration_ms": call_ms,
                        "badge": _get_thinking_badge(tc.function_name, result.get("success", False)),
                    })

                    # Observability
                    try:
                        if turn_ctx:
                            tctx = turn_ctx.new_tool_call(tc.function_name, retry_attempt=0)
            ...
            for tc in sequential_tcs:
                ...
                _seq_args_json = _json_helpers.dumps(tool_args, default=str) if tool_args else ""
                await emit("THINKING_STEP", {
                    "step_id": tool_id,
                ...
                await emit("THINKING_STEP", {
                    ...
                    "badge": _get_thinking_badge(tool_name, result.get("success", False) if isinstance(result, dict) else False),
                })
            ...
            if has_proposed and complexity != TaskComplexity.ACTION:
                ...
                logger.info(f"[MODEL] Upgraded to SELF_CORRECT at iteration {iteration + 1}")
            client, current_model = self.router.get_client_and_model(complexity)
