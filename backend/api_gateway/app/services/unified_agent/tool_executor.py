...
class ToolExecutor:
    ...
    async def _execute_get_customer_invoices(self, params: dict) -> dict:
        ...
        api_params = {"customer_id": customer_id, "limit": "20"}
        if status == "outstanding":
            api_params["status"] = "POSTED,PARTIAL"
            ...
            "results": [
                {
                    ...
                    "amount_due": str(inv.get("amount_due", inv.get("total", 0))),
    ...
    async def _execute_get_vendor_bills(self, params: dict) -> dict:
        ...
        api_params = {"vendor_id": vendor_id, "limit": "20"}
        if status == "outstanding":
            api_params["status"] = "POSTED,PARTIAL"
            ...
            "results": [
                {
                    ...
                    "total": str(b.get("total", b.get("amount", 0))),
                    "amount_paid": str(b.get("amount_paid", 0)),
                    "amount_due": str(b.get("amount_due", b.get("total", 0))),
                    "status": b.get("status", ""),
                }
                for b in bills
            ]
