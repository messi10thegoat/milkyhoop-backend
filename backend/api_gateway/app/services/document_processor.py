...
CLASSIFICATION GUIDANCE for doc_type_hint:
- "invoice": Has line_items with qty/unit_price, OR words like "faktur", "invoice", "quotation", "penawaran", "proforma", "purchase order", "PO", "nota". This includes quotations and purchase orders.
- "quotation": Specifically labeled as quotation/penawaran/proforma -- has line_items but is NOT yet a bill/invoice.
- "receipt": Payment proof, struk, kwitansi -- typically NO line_items with qty/unit_price, just a total paid.
- "bank_transfer": Screenshot of mobile banking transfer, bukti transfer bank, slip transfer ATM. Must show bank UI/app interface or transfer reference number from a bank. A regular invoice/nota with a bank account listed for payment is NOT a bank_transfer.
- "bank_statement": Tabular list of multiple transactions from a bank, rekening koran.
- "credit_note", "debit_note": Correction documents referencing original invoice.
- "tax_document": Faktur pajak with NPWP, DPP, PPN fields.
- "unknown": Cannot determine -- use this rather than guessing.

CRITICAL: If the document has line_items (multiple items with quantity and unit_price), it is almost certainly an "invoice" or "quotation", NOT a "bank_transfer". Bank transfers do not have itemized line items.

COUNTERPARTY GUIDANCE:
- "document_issuer": The company/person who ISSUED this document (printed their letterhead/logo)
- "document_recipient": The company/person this document is addressed TO (the buyer/customer)
- "counterparty_name": Keep as-is (the other party in the transaction)
- For purchase invoices/quotations FROM a vendor: document_issuer = the vendor, document_recipient = us (the buyer)
- IMPORTANT: Do NOT confuse the buyer with the seller. The letterhead/logo indicates the ISSUER.

LINE ITEM GUIDANCE:
- For each line item, extract: description, quantity, unit_price, total_price
- total_price should be the SUB TOTAL / JUMLAH for that line (qty × unit_price), NOT the unit_price repeated
- If a "SUB TOTAL" or "JUMLAH" column exists, use that value for total_price
- If total_price equals unit_price but quantity > 1, something is wrong — recalculate as qty × unit_price

Output JSON (strictly this schema, no extra fields):
{
  "doc_type_hint": "invoice" | "quotation" | "receipt" | "bank_transfer" | "bank_statement" | "credit_note" | "debit_note" | "tax_document" | "unknown",
  "confidence": "0.95",
  ...
  "document_issuer": "PT Vendor Corp" or null,
  "document_recipient": "PT Our Company" or null,
