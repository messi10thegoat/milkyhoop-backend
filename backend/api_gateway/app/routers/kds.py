...
async def create_kds_station(request: Request, data: CreateKDSStationRequest):
    ...
    async with pool.acquire() as conn:
        ...
        if exists:
            raise HTTPException(
                status_code=400, detail=f"Station code {data.code} already exists"
            )
        ...
        """,
            ...
            data.auto_bump_minutes,
            data.alert_threshold_minutes,
            "fifo",
            ctx["user_id"],
        )

        return KDSResponse(
            success=True, message="KDS station created", data={"id": str(row["id"])}
        )


@router.put("/stations/{station_id}", response_model=KDSResponse)
async def update_kds_station(
    request: Request, station_id: UUID, data: UpdateKDSStationRequest
):
    ...
    async with pool.acquire() as conn:
        ...
        async with conn.transaction():
            ...
            order_number = await conn.fetchval(
                "SELECT generate_kds_order_number($1)", ctx["tenant_id"]
            )
...
async def get_kds_order(request: Request, order_id: UUID):
    ...
    async with pool.acquire() as conn:
        ...
        elapsed = (
            int((datetime.now() - order["created_at"]).total_seconds())
            if order["created_at"]
            else 0
        )
...
async def list_kds_alerts(
    ...
    async with pool.acquire() as conn:
        ...
        return KDSAlertListResponse(
            items=items, total=len(items), unacknowledged=unacked
        )
...
async def get_kds_metrics(
    ...
    async with pool.acquire() as conn:
        ...
        avg_time = (
            sum(m["avg_ticket_time_seconds"] for m in metrics) / len(metrics)
            if metrics
            else 0
        )
