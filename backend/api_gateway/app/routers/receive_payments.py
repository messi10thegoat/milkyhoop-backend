...
async def list_receive_payments(
    ...
    Law 29: Amounts derived from journal_lines (RECEIVABLE credits).
    LEFT JOIN to receive_payments wrapper for metadata.
    Includes draft payments from wrapper table (no journal yet).
    Excludes reversed journals via reversed_by_id IS NULL.
    """
    try:
        ctx = get_user_context(request)
        pool = await get_pool()
        ...
        async with pool.acquire() as conn:
            ...
            # Status filter applied on the unified result
            ...
            if customer_id:
                outer_conditions.append(f"q.customer_id = ${param_idx}")
            ...
            if source_type and source_type != "all":
                outer_conditions.append(f"q.source_type = ${param_idx}")
            ...
            sort_field = sort_mapping.get(sort_by, "q.created_at")
            sort_dir = "DESC" if sort_order == "desc" else "ASC"

            # ---------------------------------------------------------------
            # Unified query: journal-derived settlements + draft wrappers
            # ---------------------------------------------------------------
            ...
            base_query = f"""
                WITH settlements AS (
                    SELECT je.id AS journal_id, je.journal_number, je.journal_date,
                           je.source_id, je.source_type AS je_source_type,
                           je.created_at AS je_created_at,
                           SUM(jl.credit) AS settlement_amount
            ...
            # Execute count (pass all params except limit/offset)
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                # (1) DOMAIN lock per-customer — LAPIS KEDUA, bukan penjaga utama.
                #     ⚠️ KOREKSI 2026-08-06: pada jalur AP yang setara, uji-merah
                #     membuktikan anti-over-application dijaga oleh row-lock
                #     `FOR UPDATE` atas dokumen + remaining journal-derived, BUKAN
                #     oleh advisory lock ini. Jalur AR belum diuji-merah setara —
                #     perlakukan pembenarannya sebagai BELUM TERBUKTI, bukan fakta.
                #     Dipertahankan: nol biaya, pertahanan berlapis.
                #     Tetap JANGAN diganti idem_key — alokasi berbeda punya kunci
                #     berbeda, jadi idem_key tak pernah menjadi pengganti yang sah
                #     untuk lock domain (LOCK cegah RACE, IDEMPOTENCY cegah DUPLIKAT).
...
async def get_receive_payment_journal_entries(request: Request, payment_id: str):
    """
    Tab: Journal Entries - Get journal entries linked to this receive payment.
    Returns both the posting journal and void/reversal journal if any.
    """
    try:
        try:
        ...
        async with pool.acquire() as conn:
            payment = await conn.fetchrow(
                """
                SELECT id, payment_number, journal_id, void_journal_id, status
