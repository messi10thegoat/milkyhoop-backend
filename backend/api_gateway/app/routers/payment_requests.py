...
async def list_payment_requests(
    ...
    from backend.api_gateway.app.services.payment_request_service import get_payment_request_service
...
async def approve_payment_request(request: Request, request_id: str, data: ApproveRejectDTO = None):
    ...
    if business_role and business_role not in ["OWNER", "FINANCE_MGR"]:
        raise HTTPException(status_code=403, detail="Only Owner or Finance Manager can approve")
...
async def reject_payment_request(request: Request, request_id: str, data: ApproveRejectDTO):
    ...
    business_role = user.get("business_role_code")
    if business_role and business_role not in ["OWNER", "FINANCE_MGR"]:
        raise HTTPException(status_code=403, detail="Only Owner or Finance Manager can reject")
...
async def mark_payment_request_paid(request: Request, request_id: str, data: MarkPaidDTO):
    """
    ...
    business_role = user.get("business_role_code")
    if business_role and business_role not in ["OWNER", "FINANCE_MGR"]:
        raise HTTPException(status_code=403, detail="Only Owner or Finance Manager can mark as paid")
...
async def get_payment_request_journal_entries(request: Request, request_id: str):
    ...
    try:
        ...
        logger.error(f"Error getting payment request journal entries: {e}", exc_info=True)
