        ...
        async with pool.acquire() as conn:
            ...
            if search:
                ...
                if len(words) == 1:
                    ...
                    word_conds = []
                    for word in words:
                        word_conds.append(f"bt.transfer_number ILIKE ${param_idx}")
...
async def get_bank_transfer_journal_entries(request: Request, transfer_id: str):
    """
    Tab: Journal Entries - Get journal entries linked to this bank transfer.
    Includes the original posting journal and any reversal journal.
    """
    try:
        try:
            uuid_module.UUID(transfer_id)
        ...
        async with pool.acquire() as conn:
            transfer = await conn.fetchrow(
                """
                SELECT id, transfer_number, journal_id, status
                FROM bank_transfers
            ...
            if transfer["journal_id"]:
                journal_ids.append(transfer["journal_id"])
        ...
        logger.error(f"Error getting bank transfer journal entries: {e}", exc_info=True)
