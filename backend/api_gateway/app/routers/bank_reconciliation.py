...
from ..schemas.bank_reconciliation import (
    StartReconciliationRequest, UpdateReconciliationRequest,
    MatchTransactionsRequest, UnmatchTransactionsRequest, AdjustmentRequest,
    ReconciliationListResponse, ReconciliationDetailResponse, ReconciliationResponse,
    ReconciliationSummaryResponse, UnreconciledTransactionsResponse,
    ReconciliationHistoryResponse, ReconciliationListItem, ReconciliationDetail,
    ReconciliationItem, ReconciliationSummary, UnreconciledTransaction
)
...
async def list_reconciliations(
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            total = await conn.fetchval(f"SELECT COUNT(*) FROM bank_reconciliations br WHERE {where_clause}", *params)
...
async def get_reconciliation_detail(request: Request, recon_id: str):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            """, uuid_module.UUID(recon_id), ctx['tenant_id'])
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                """, uuid_module.UUID(body.bank_account_id), ctx['tenant_id'])
                ...
                if existing:
                    raise HTTPException(status_code=400, detail="Bank account has an in-progress reconciliation")

                recon_number = await conn.fetchval(
                    "SELECT generate_reconciliation_number($1, 'REC')",
                    ctx['tenant_id']
                )
                ...
                """, recon_id, ctx['tenant_id'], uuid_module.UUID(body.bank_account_id),
                    recon_number, body.statement_date, body.statement_start_date,
                    body.statement_end_date, body.statement_opening_balance,
                    body.statement_closing_balance, system_balance, ctx['user_id'])
                ...
                """, uuid_module.UUID(body.bank_account_id),
                    body.statement_start_date, body.statement_end_date)

                for txn in transactions:
                    await conn.execute("""
                        INSERT INTO bank_reconciliation_items (id, reconciliation_id, bank_transaction_id)
                        VALUES ($1, $2, $3)
                    """, uuid_module.uuid4(), recon_id, txn['id'])
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if recon['status'] != 'in_progress':
                raise HTTPException(status_code=400, detail="Only in-progress reconciliations can be deleted")

            await conn.execute("DELETE FROM bank_reconciliations WHERE id = $1", uuid_module.UUID(recon_id))
...
async def match_transactions(request: Request, recon_id: str, body: MatchTransactionsRequest):
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                for txn_id in body.transaction_ids:
                    ...
                    """, uuid_module.UUID(recon_id), uuid_module.UUID(txn_id), ctx['user_id'])
...
async def unmatch_transactions(request: Request, recon_id: str, body: UnmatchTransactionsRequest):
            ...
            async with conn.transaction():
                ...
                for txn_id in body.transaction_ids:
                    await conn.execute("""
                        ...
                        WHERE reconciliation_id = $1 AND bank_transaction_id = $2
                    """, uuid_module.UUID(recon_id), uuid_module.UUID(txn_id))
    ...
    try:
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                """, uuid_module.UUID(recon_id), ctx['user_id'])
                ...
                await conn.execute("""
                    ...
                    WHERE id = $1
                """, recon['bank_account_id'], recon['statement_date'], recon['statement_closing_balance'])
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                if recon['status'] != 'completed':
                    raise HTTPException(status_code=400, detail="Only completed reconciliations can be voided")
...
async def get_unreconciled_transactions(request: Request, recon_id: str):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail="Failed to get unreconciled transactions")
