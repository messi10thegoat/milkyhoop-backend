...
async def create_currency(request: Request, body: CreateCurrencyRequest):
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            if existing:
                raise HTTPException(
                    status_code=400, detail=f"Currency {body.code} already exists"
                )
...
async def update_currency(
    request: Request, currency_id: str, body: UpdateCurrencyRequest
):
    """Update a currency."""
    try:
        ctx = get_user_context(request)
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            for cid in [body.from_currency_id, body.to_currency_id]:
                ...
                if not c:
                    raise HTTPException(
                        status_code=400, detail=f"Currency {cid} not found"
                    )

            if body.from_currency_id == body.to_currency_id:
                raise HTTPException(
                    status_code=400, detail="From and To currencies must be different"
                )
            ...
            """,
                rate_id,
                ctx["tenant_id"],
                uuid_module.UUID(body.from_currency_id),
                uuid_module.UUID(body.to_currency_id),
                body.rate_date,
                body.rate,
                body.source,
                ctx["user_id"],
            ...
            return ExchangeRateResponse(
                ...
                data={
                    "id": str(rate_id),
                    "rate": body.rate,
                    "rate_date": body.rate_date.isoformat(),
                },
            )
    except HTTPException:
        raise
    ...
    try:
        ...
        async with pool.acquire() as conn:
            ...
            from_c = await conn.fetchrow(
                ...
                uuid_module.UUID(from_currency_id),
            )
            to_c = await conn.fetchrow(
                "SELECT code FROM currencies WHERE id = $1",
                uuid_module.UUID(to_currency_id),
            )
