...
async def _fetch_group_items(conn, group_id: UUID) -> list:
    ...
    return [
        {
            "id": str(r["id"]),
            "tax_group_id": str(r["tax_group_id"]),
            "tax_code_id": str(r["tax_code_id"]),
            "tax_name": r["tax_name"],
            "rate": float(r["rate"]),
            "sequence": r["sequence"],
        }
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                if existing:
                    raise HTTPException(
                        status_code=400,
                        detail=f"Group code '{body.code}' already exists",
                    )
...
async def update_tax_group(
    request: Request, group_id: UUID, body: UpdateTaxGroupRequest
):
    try:
        ctx = get_user_context(request)
        pool = await get_pool()
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                if update_data:
                    if (
                        "code" in update_data
                        and update_data["code"] != existing["code"]
                    ):
                        ...
                        if dup:
                            raise HTTPException(
                                status_code=400,
                                detail=f"Group code '{update_data['code']}' already exists",
                            )
                    ...
                    await conn.execute(
                        f"UPDATE tax_groups SET {', '.join(sets)} WHERE id = ${idx} AND tenant_id = ${idx + 1}",
                        *params,
                    )
                ...
                if body.items is not None:
                    await conn.execute(
                        "DELETE FROM tax_group_items WHERE tax_group_id = $1", group_id
                    )
                    for item in body.items:
                        await conn.execute(
                            """
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                if not existing:
                    raise HTTPException(status_code=404, detail="Tax group not found")
                await conn.execute(
                    "DELETE FROM tax_group_items WHERE tax_group_id = $1 AND tenant_id = $2",
                    group_id,
                    ctx["tenant_id"],
                )
                await conn.execute(
                    "DELETE FROM tax_groups WHERE id = $1 AND tenant_id = $2",
                    group_id,
                    ctx["tenant_id"],
                )
                return {
                    "success": True,
                    "message": "Tax group deleted",
                    "data": {"id": str(group_id)},
                }
    except HTTPException:
        raise
