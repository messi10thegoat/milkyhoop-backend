...
class SuperIntelligentConfidenceEngine:
    ...
    def calculate_super_confidence(self, query: str, faq_results: List = None, tenant_id: str = "default") -> float:
        ...
        if faq_results and len(faq_results) > 0:
            ...
            faq_content = f"{best_faq.get('question', '')} {best_faq.get('answer', '')}"
            ...
            token_ratio = fuzz.token_sort_ratio(query_lower, faq_question.lower()) / 100.0
            string_similarity = max(partial_ratio, token_ratio)
            
            # High similarity = high confidence
            if string_similarity > 0.7:
                confidence = 0.85 + (string_similarity - 0.7) * 0.5  # 0.85-1.0 range
                logger.info(f"[{tenant_id}] High FAQ similarity: {string_similarity:.3f}")
            elif string_similarity > 0.5:
                confidence = 0.60 + (string_similarity - 0.5) * 1.25  # 0.60-0.85 range
                logger.info(f"[{tenant_id}] Medium FAQ similarity: {string_similarity:.3f}")
            else:
                confidence = string_similarity * 0.6  # 0.0-0.30 range
                logger.info(f"[{tenant_id}] Low FAQ similarity: {string_similarity:.3f}")
...
- Gunakan HANYA informasi dari FAQ di atas
- DILARANG menambah info dari pengetahuan umum
- Berikan analisis yang cerdas dan helpful
- Bandingkan opsi jika relevan dengan data FAQ
- Maksimal 3 kalimat, natural dan informatif
- Jika info tidak ada di FAQ: "Informasi tersebut tidak tersedia"

RESPONS CERDAS:"""

        else:  # deep understanding
            prompt = f"""Anda adalah senior AI consultant super intelligent untuk {tenant_id}. Berikan analisis mendalam berdasarkan FAQ lengkap.
...
- Analisis mendalam menggunakan HANYA data FAQ
- DILARANG hallucination atau info external
- Berikan rekomendasi terbaik dari opsi yang tersedia
- Jelaskan keuntungan/pertimbangan dengan data spesifik
- Maksimal 4 kalimat, terstruktur dan comprehensive
- Prioritaskan akurasi di atas kelengkapan
...
ANALISIS MENDALAM:"""
        ...
        try:
            ...
            logger.info(f"[{tenant_id}] Super {intelligence_level} synthesis completed")
            return result
